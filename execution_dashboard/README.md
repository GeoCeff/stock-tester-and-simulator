...
Live submission remains blocked unless the server was started with `ENABLE_LIVE_ORDERS=1`, IBKR is authenticated, an account is selected, conid is resolved, readiness gates pass, the exact `LIVE SYMBOL` confirmation reaches the backend for a manual order, and the backend order-ticket validator accepts the ticket. For both manual and automated orders, the backend permits only one live submission in flight, rejects research/model risk settings above 1%, verifies the exact IBKR stock contract, enforces `DAY` for day trades and `GTC` otherwise, requires current P&L and account evidence, available funds, no duplicate position or active order, a spread no wider than 20 bps, daily loss above -2%, and quantity within the published stop-risk budget. It rebuilds the broker payload from an allowlist after validation, discarding any extra request properties. The dashboard records a submitted order only after every IBKR response row includes an order ID. Failed, malformed, and precautionary-warning responses remain unsubmitted; warnings cannot be acknowledged through this server. Every live-order intent/response is appended to `data/audit.jsonl`.
