...
Live submission remains blocked unless the server was started with `ENABLE_LIVE_ORDERS=1`, IBKR is authenticated, an account is selected, conid is resolved, readiness gates pass, the typed confirmation matches the selected symbol, and the backend order-ticket validator accepts the ticket. The backend verifies that every bracket leg uses the same IBKR stock contract returned for the exact candidate symbol, enforces `DAY` for day trades and `GTC` otherwise, fetches current net liquidation, and rejects missing account data or any quantity whose stop risk exceeds the published candidate risk budget. It rebuilds the broker payload from an allowlist after validation, discarding any extra request properties. Every live-order intent/response is appended to `data/audit.jsonl`.
